...
# Remove the breaking ticker completely; keep one clear latest-news bar.
s=re.sub(r'<div\s+class="breakingTicker"[^>]*>.*?</div>\s*', '', s, count=1, flags=re.S)
# Remove prior ticker style/runtime and rebuild a single slow ticker controller.
...
for sid in ('force-live-ticker-runtime','live-news-ticker-runtime','final-ticker-runtime'):
    s=re.sub(r'<script id="'+sid+r'">.*?</script>', '', s, flags=re.S)
...
(function(){
 ...
 let set=null,width=0,x=0,last=0,signature=''; const speed=24;
 function render(items){
   const clean=(items||[]).filter(x=>x&&x.title).slice(0,48); if(!clean.length)return;
   const sig=clean.map(x=>(x.title||'')+'|'+(x.url||'')).join('||'); if(sig===signature&&set)return; signature=sig;
   track.innerHTML=''; set=document.createElement('div'); set.className='tickerSet';
   ...
   const clone=set.cloneNode(true);track.append(set,clone); x=0; requestAnimationFrame(()=>{width=set.getBoundingClientRect().width;});
 }
...
p.write_text(s,encoding='utf-8'); print('SINGLE SLOW LATEST-NEWS TICKER OK')
