...
for item in items:
    ...
    fallback_chain = [u for u in (raw_image, raw_original, original) if u and u != primary]
    fallback_js = 'this.dataset.failed=(Number(this.dataset.failed||0)+1);if(Number(this.dataset.failed)===1&&this.dataset.raw){this.src=this.dataset.raw;return;}if(Number(this.dataset.failed)===2&&this.dataset.original){this.src=this.dataset.original;return;}this.removeAttribute(\'src\');this.classList.add(\'image-unavailable\');'
    ld = {
...
print(f'تم إنشاء {len(urls)} صفحة خبر مع صورة القالب وسلسلة بدائل للصورة نفسها عند تعذر التحميل')
