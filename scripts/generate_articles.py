...
PLACEHOLDER = BASE + 'assets/news/no-image-default.svg'
...
def image_url(path):
    p = str(path or '')
    if not p or p.lower().endswith('.svg') or '6deaa228-fef4-472c-819d-400fa6c78630.jpg' in p: return PLACEHOLDER
...
for u,pub,title,cat,image in urls[:30]:
    enclosure='' if image.lower().endswith('.svg') else f'<enclosure url="{esc(image)}" type="image/jpeg" />'
...
print(f'تم إنشاء {len(urls)} صفحة خبر بدون عرض مصدر الخبر')
