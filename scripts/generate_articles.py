...
def image_url(path):
    p = str(path or '')
    if not p or p.lower().endswith('.svg') or '6deaa228-fef4-472c-819d-400fa6c78630.jpg' in p:
        return ''
...
urls = []
for item in items:
    sid = slug(item); article_url = f'{BASE}news/{sid}.html'; item['article_url'] = article_url
    title = item.get('title', 'خبر من التاسعة نيوز'); cat = item.get('category', 'أخبار العراق'); published = str(item.get('published') or '')
    image = image_url(item.get('image'))
    original = image_url(item.get('original_image'))
    primary = image or original
    ld = {'@context':'https://schema.org','@type':'NewsArticle','headline':title,'datePublished':published,'dateModified':published,'mainEntityOfPage':{'@type':'WebPage','@id':article_url},'image':[primary] if primary else [],'articleSection':cat,'author':{'@type':'Organization','name':'التاسعة نيوز'},'publisher':{'@type':'Organization','name':'التاسعة نيوز','logo':{'@type':'ImageObject','url':BASE+'assets/logo.svg'}},'description':title,'isPartOf':{'@type':'NewsMediaOrganization','name':'التاسعة نيوز','url':BASE}}
    fallback_attr = f" onerror=\"this.onerror=null;this.src='{esc(original)}'\"" if original and original != primary else ''
    hero_attr = f' src="{esc(primary)}"' if primary else ''
    doc = f'''<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index,follow,max-image-preview:large"><title>{esc(title)} | التاسعة نيوز</title><meta name="description" content="{esc(title)}"><link rel="canonical" href="{esc(article_url)}"><meta property="og:type" content="article"><meta property="og:site_name" content="التاسعة نيوز"><meta property="og:title" content="{esc(title)}"><meta property="og:description" content="{esc(title)}"><meta property="og:url" content="{esc(article_url)}"><meta property="og:image" content="{esc(primary)}"><meta property="og:locale" content="ar_IQ"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{esc(title)}"><meta name="twitter:image" content="{esc(primary)}"><script type="application/ld+json">{json.dumps(ld, ensure_ascii=False, separators=(',',':'))}</script><style>body{{margin:0;background:#f4f6f9;color:#14263b;font-family:Tahoma,Arial,sans-serif}}.wrap{{width:min(900px,calc(100% - 28px));margin:auto}}header{{background:#071b33;border-bottom:4px solid #e21d2b;padding:18px 0}}header a{{color:#fff;text-decoration:none;font-weight:900}}main{{padding:28px 0 50px}}article{{background:#fff;border:1px solid #dfe5ec;border-radius:14px;overflow:hidden}}.hero{{width:100%;height:min(500px,55vw);min-height:240px;object-fit:cover;display:block;background:#dbe4ee}}.content{{padding:25px}}.tag{{display:inline-block;background:#e21d2b;color:#fff;padding:7px 11px;border-radius:5px;font-size:12px;font-weight:900}}h1{{font-size:30px;line-height:1.6;margin:14px 0}}.meta{{color:#718096;font-size:12px;margin-bottom:22px}}p{{font-size:17px;line-height:2;color:#33485e}}footer{{text-align:center;color:#718096;padding:20px;font-size:11px}}</style></head><body><header><div class="wrap"><a href="{BASE}">التاسعة نيوز — نعلم لتعلم</a></div></header><main><div class="wrap"><article><img class="hero"{hero_attr} alt="{esc(title)}" loading="eager" decoding="async"{fallback_attr}><div class="content"><span class="tag">{esc(cat)}</span><h1>{esc(title)}</h1><div class="meta">تاريخ النشر: {esc(published)}</div><p>تنشر <strong>التاسعة نيوز</strong> هذا الخبر ضمن تغطيتها المستمرة للأحداث في العراق وكركوك والعالم.</p></div></article></div></main><footer>© التاسعة نيوز — نعلم لتعلم</footer></body></html>'''
    (NEWS_DIR / f'{sid}.html').write_text(doc, encoding='utf-8'); urls.append((article_url,published,title,cat,primary))
...
for u,pub,_,_,_ in urls: sitemap.append(f'<url><loc>{esc(u)}</loc><lastmod>{esc(pub)}</lastmod><changefreq>daily</changefreq><priority>0.8</priority></url>')
sitemap.append('</urlset>'); Path('sitemap.xml').write_text('\n'.join(sitemap)+'\n',encoding='utf-8')
...
rss.append('</channel></rss>'); Path('feed.xml').write_text('\n'.join(rss)+'\n',encoding='utf-8')
print(f'تم إنشاء {len(urls)} صفحة خبر مع صورة القالب ثم الصورة الأصلية عند الحاجة')
