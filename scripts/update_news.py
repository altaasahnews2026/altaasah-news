...
def page_candidates(url):
    ...
    except Exception as e:print("تعذر تحليل صفحة المصدر:",e)
    return unique(out)
...
def save_image(url):
    ...
    except Exception as e:print("تعذر حفظ الصورة:",e);return ""

def neutral_placeholder(title):
    digest=hashlib.sha256(title.encode()).hexdigest()[:16];name=f"no-image-{digest}.svg";path=os.path.join(NEWS_IMAGE_DIR,name)
    if not os.path.exists(path):
        with open(path,"w",encoding="utf-8") as f:f.write('<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="700"><rect width="1200" height="700" fill="#edf1f5"/><rect x="70" y="70" width="1060" height="560" rx="28" fill="#dce4ec"/><text x="600" y="370" text-anchor="middle" font-family="Arial" font-size="36" fill="#34495e">لا تتوفر صورة أصلية لهذا الخبر</text></svg>')
    return "./assets/news/"+name

items=[];seen_titles=set()
for query in QUERIES:
    params=urllib.parse.urlencode({"q":f"{query} when:1d","hl":"ar","gl":"IQ","ceid":"IQ:ar"})
    try:raw,_=fetch("https://news.google.com/rss/search?"+params);root=ET.fromstring(raw)
    except Exception as e:print("تعذر جلب الأخبار:",e);continue
    for item in root.findall("./channel/item"):
        ...
        seen_titles.add(title);cat=category(title);source=publisher_url(item)
        # First try images explicitly embedded in the RSS item, then resolve the Google News article URL itself.
...
if not items:print("لا توجد أخبار جديدة");raise SystemExit(0)
