...
ROOT=Path('.'); NEWS=ROOT/'news.json'; OUT=ROOT/'assets/news'
...
# لا نركب أي صورة قبل اجتياز بوابة اختيار الصور ومنع التكرار.
runpy.run_path('scripts/image_quality_gate2.py',run_name='__main__')
def font(size):
...
def source_path(v):
 p=str(v or '');p=p[2:] if p.startswith('./') else p
 q=ROOT/p if p.startswith('assets/') else None
 return q if q and q.exists() and q.is_file() else None
...
for item in items:
 src=item.get('image') or item.get('original_image');sp=source_path(src)
...
NEWS.write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8');print('تم تركيب صور منتقاة داخل القالب على',len(items),'خبراً')
