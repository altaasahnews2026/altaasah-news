...
def build(template, photo):
    ...
    # Put the real news photo into the image area only. No headline/caption,
    # dark panel, or text is drawn over the photo.
...
for item in items:
    ...
    try:
        with Image.open(sp) as photo:
            ...
            build(template, photo).save(out, quality=88, optimize=True, progressive=True)
...
print(f'تم تطبيق قالب التاسعة نيوز على {count} خبراً بدون كابشن فوق الصورة')
