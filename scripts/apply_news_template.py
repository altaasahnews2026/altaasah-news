...
def official_template():
 W=H=1254; im=Image.new('RGB',(W,H),(5,20,39)); d=ImageDraw.Draw(im)
 d.rectangle((0,0,W,245),fill=(210,25,38)); d.text((W//2,112),shape_ar('خبر'),font=font(92),fill='white',anchor='mm')
 for x in range(40,W,55):
  for y in range(250,950,55):
...
def fit_cover(photo,size):
 tw,th=size; photo=photo.convert('RGB'); s=max(tw/photo.width,th/photo.height)
 nw=max(tw,round(photo.width*s)); nh=max(th,round(photo.height*s)); photo=photo.resize((nw,nh),Image.Resampling.LANCZOS)
 return photo.crop(((nw-tw)//2,(nh-th)//2,(nw-tw)//2+tw,(nh-th)//2+th))
def build(template,photo):
 photo=fit_cover(photo,(1254,705)).filter(ImageFilter.UnsharpMask(radius=1,percent=80,threshold=3))
 c=template.copy(); c.paste(photo,(0,245)); return c.convert('RGB')
...
for item in items:
 ...
 key=hashlib.sha1((str(item.get('url',''))+'|'+str(item.get('title',''))).encode()).hexdigest()[:16]
 out=OUT/f'news-template-{key}.jpg'
 try:
  with Image.open(sp) as photo:
   photo.load(); build(template,photo).save(out,quality=90,optimize=True,progressive=True)
  with Image.open(out) as check:
   check.verify(); assert check.size==(1254,1254) and check.mode=='RGB'
...
print('تم تركيب صور الأخبار داخل القالب:',len(items))
